...
// Set test environment before other imports to prevent index.ts from auto-listening
process.env.NODE_ENV = 'test';
process.env.PORT = '3001';

import app from '../index';
import { dapatkanKoneksiDatabase, tutupKoneksiDatabase } from '@infrastruktur/database';
...
const results: { endpoint: string; status: 'PASS' | 'FAIL'; message: string }[] = [];
...
async function runTests() {
  ...
  // Response structure: { status: 'success', data: { token: '...' } }
...
async function main() {
  try {
    ...
    console.log('\n--- TEST SUMMARY ---');
    results.forEach(r => console.log(`[${r.status}] ${r.endpoint}`));
  } catch (error) {
